...
- [x] Task 5: Implement batch sync orchestration action (AC: 3.5.6, 3.5.7, 3.5.8)
  ...
  - [x] For each batch and each provider:
    - Create marketplace client using factory from `convex/marketplaces/shared/helpers.ts`
...
- Marketplace client factories: `createBricklinkStoreClient`, `createBrickOwlStoreClient` in `convex/marketplaces/shared/helpers.ts`
...
```typescript
// Factory functions from convex/marketplaces/shared/helpers.ts:
...
- Implemented `batchSyncFile` internal action:
  ...
  - **Client creation**: Uses factory functions from `convex/marketplaces/shared/helpers.ts`
