...
import { withBoClient, createBoRequestState } from "../client";
...
export async function executeBulkRequests(
  ...
  return await withBoClient(ctx, {
    businessAccountId,
    fn: async (client) => {
      const requestState = createBoRequestState();
        ...
        try {
          ...
          response.forEach((entry, requestIndex) => {
            ...
            if (isErrorResponse(entry)) {
              const normalized = normalizeBoStoreError(entry);
              failed += 1;
              results.push({
                success: false,
                error: normalized,
          ...
          const normalized = normalizeBoStoreError(rawError);
