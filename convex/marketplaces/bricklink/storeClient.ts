...
export class BricklinkStoreClient {
  ...
  private async makeRequest<T>(
    ...
    await this.ctx.runMutation(internal.marketplaces.shared.mutations.resetFailures, {
      businessAccountId: this.businessAccountId,
      provider: "bricklink",
    });

    // Check if alert threshold was reached and emit metric
    const updatedQuota = await this.ctx.runQuery(
      internal.marketplaces.shared.mutations.getQuotaState,
      {
        businessAccountId: this.businessAccountId,
        provider: "bricklink",
      },
