...
export interface OrderPageResult {
  items: BLOrderSummaryResponse[];
  meta: BricklinkApiResponse<BLOrderSummaryResponse[]>["meta"];
}
...
export async function listOrders(
  ...
  const page = await listOrdersPage(ctx, params);
  return page.items;
}
...
export async function listOrdersPage(
  ...
  const filters = params.filters
    ? blOrdersListFiltersSchema.parse(params.filters)
    : undefined;
  ...
  return await withBricklinkClient(ctx, {
    ...
    fn: async (client) => {
      ...
      const response = await client.request<BricklinkApiResponse<BLOrderSummaryResponse[]>>({
        path: "/orders",
        method: "GET",
        query,
      });
      ...
      const items = itemsRaw.map((item) => blOrderSummaryResponseSchema.parse(item));
...
export async function getOrder(
  ...
  return await withBricklinkClient(ctx, {
    businessAccountId,
    fn: async (client) => {
      const response = await client.request<BricklinkApiResponse<BLOrderResponse>>({
        path: `/orders/${orderId}`,
        method: "GET",
      });
...
export async function getOrderItems(
  ...
  return await withBricklinkClient(ctx, {
    businessAccountId,
    fn: async (client) => {
      const response = await client.request<BricklinkApiResponse<BLOrderItemResponse[][]>>({
      ...
      return batches.map((batch) =>
        batch.map((item) => blOrderItemResponseSchema.parse(item)),
      );
    },
  });
}
...
export async function sendDriveThruEmail(
  ...
  const options = params.options
    ? blDriveThruOptionsSchema.parse(params.options)
    : undefined;
