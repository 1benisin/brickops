...
import { formatApiError } from "../../inventory/helpers";
import {
  getSyncStateForItem,
} from "./helpers";
  ...
  try {
    ...
    if (result.success) {
      ...
      if (message.attempt >= 4) {
        // This is the 5th attempt (0-indexed), mark as permanently failed
    ...
    // Check if we've exceeded max retries (attempt is 0-indexed, so 4 = 5th attempt)
    if (message.attempt >= 4) {
