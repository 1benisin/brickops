...
You call your Convex functions via [client libraries](https://docs.convex.dev/client/react) or directly via [HTTP](about:/http-api/#functions-api).
...
Convex provides client libraries for nearly all popular web and native app frameworks. Client libraries connect to your Convex deployment via WebSockets. You can then call your public Convex functions [through the library](about:/client/react#fetching-data). You can also use Convex with [HTTP directly](about:/http-api/#functions-api), you just won't get the automatic subscriptions.
