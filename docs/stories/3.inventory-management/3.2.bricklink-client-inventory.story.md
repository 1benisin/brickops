...
```typescript
// convex/marketplaces/bricklink/storeClient.ts
...
```typescript
// convex/marketplaces/bricklink/catalogClient.ts (renamed from bricklinkClient.ts, refactored)
import { generateOAuthSignature, buildAuthorizationHeader } from "./oauth";
...
- **Shared OAuth**: `convex/marketplaces/bricklink/oauth.ts` (new file - pure OAuth 1.0a signing functions extracted from existing client)
- **Catalog Client** (Story 2.3): `convex/marketplaces/bricklink/catalogClient.ts` (renamed from `bricklinkClient.ts`, refactored to use shared OAuth helpers, export renamed to `catalogClient` - **NO breaking changes to functionality**)
- **Store Client** (This Story): `convex/marketplaces/bricklink/storeClient.ts` (new file - uses shared OAuth + DB rate limiting for store operations)
- **Store Mappers**: `convex/marketplaces/bricklink/storeMappers.ts` (new file - bidirectional BrickLink store ↔ Convex mapping, handles inventory now + orders in Epic 4)
- **Marketplace Functions**: `convex/marketplaces/shared/helpers.ts` (extend with `createBricklinkStoreClient()` factory + rate limiting mutations/queries)
- **Marketplace Types**: `convex/marketplaces/shared/types.ts` (new file - shared interfaces across all marketplace providers)
...
```typescript
// Defined in convex/marketplaces/shared/types.ts
...
- Created `convex/marketplaces/bricklink/oauth.ts` with shared OAuth 1.0a signing functions:
...
- Refactored `convex/marketplaces/bricklink/catalogClient.ts` (renamed from `bricklinkClient.ts`) to use shared OAuth module:
...
- Created `convex/marketplaces/bricklink/storeClient.ts` with database-backed rate limiting:
...
- Added TypeScript interfaces to `convex/marketplaces/bricklink/storeClient.ts`:
...
- Created `convex/marketplaces/bricklink/storeMappers.ts` with bidirectional mappers:
...
- Added bulk operation types to `convex/marketplaces/bricklink/storeClient.ts`:
...
- Added retry configuration types to `convex/marketplaces/bricklink/storeClient.ts`:
...
- Created `convex/marketplaces/shared/types.ts` with shared interfaces:
...
- `convex/marketplaces/bricklink/oauth.ts` (112 lines)
- `convex/marketplaces/bricklink/storeClient.ts` (1,283 lines - complete with CRUD, bulk, rate limiting, retry, metrics, dry-run)
- `convex/marketplaces/bricklink/storeMappers.ts` (251 lines - bidirectional data mappers)
- `convex/marketplaces/shared/types.ts` (89 lines - shared interfaces for Story 3.4 integration)

**Modified:**

- `convex/marketplaces/bricklink/catalogClient.ts` (renamed from `bricklinkClient.ts`, refactored OAuth signing, export renamed to `catalogClient`, 422 lines)
