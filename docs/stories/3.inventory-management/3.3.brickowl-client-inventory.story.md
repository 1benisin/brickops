...
```typescript
{
  ...
  bricklinkLotId?: number; // From Story 3.2 (track BrickLink sync)
  createdBy: Id<"users">;
  createdAt: number;
  updatedAt?: number;
  isArchived?: boolean;
...
Note that `inventoryItems` will have **both** `brickOwlLotId` and `bricklinkLotId` fields. This enables:
...
- **Auth Module**: `convex/marketplaces/brickowl/auth.ts` (new file - API key authentication helpers)
- **Store Client** (This Story): `convex/marketplaces/brickowl/storeClient.ts` (new file - uses auth helpers + DB rate limiting for store operations)
- **Store Mappers**: `convex/marketplaces/brickowl/storeMappers.ts` (new file - bidirectional BrickOwl store ↔ Convex mapping, handles inventory now + orders in Epic 4)
- **Marketplace Functions**: `convex/marketplaces/shared/helpers.ts` (extend with `createBrickOwlStoreClient()` factory)
- **Marketplace Types**: `convex/marketplaces/shared/types.ts` (extend if needed - reuse existing `StoreOperationResult` interface)
...
```typescript
// Defined in convex/marketplaces/shared/types.ts (from Story 3.2)
interface StoreOperationResult {
  success: boolean;
  brickowlLotId?: string; // BrickOwl uses string lot_id
  bricklinkLotId?: number; // BrickLink uses number inventory_id
  error?: {
...
7. ✅ Convex schema tracks both `bricklinkLotId` and `brickowlLotId` for multi-provider sync
...
- Created `convex/marketplaces/brickowl/auth.ts` with pure, testable authentication helpers:
...
- Created `convex/marketplaces/brickowl/storeClient.ts` following BrickLink client pattern:
...
- Updated `convex/marketplaces/shared/types.ts` to include both brickowlLotId and brickowlInventoryId fields
...
- Created `convex/marketplaces/brickowl/storeMappers.ts` with comprehensive bidirectional mapping functions:
...
- **REFACTORED**: Removed duplicate `ConvexInventoryItem` interface from both BrickLink and BrickOwl mappers
  ...
  - Refactored both `convex/marketplaces/bricklink/storeMappers.ts` and `convex/marketplaces/brickowl/storeMappers.ts`
...
- Rollback patterns fully documented in `convex/marketplaces/brickowl/storeClient.ts` file header (lines 5-37):
...
- `StoreOperationResult` interface already includes rollbackData field:
  - ✅ Reuses existing interface from `convex/marketplaces/shared/types.ts` (Story 3.2)
...
- `convex/marketplaces/brickowl/auth.ts`
- `convex/marketplaces/brickowl/storeClient.ts`
- `convex/marketplaces/brickowl/storeMappers.ts`

**Modified:**

- `convex/functions/marketplace.ts` (added BrickOwl client factory; updated rate limiting for BrickOwl)
- `convex/marketplaces/shared/types.ts` (added brickowlLotId field)
- `convex/schema.ts` (added brickowlLotId field and index to inventoryItems table)
- `convex/marketplaces/bricklink/storeMappers.ts` (refactored to use Doc<"inventoryItems"> instead of duplicate interface)
- `convex/marketplaces/brickowl/storeMappers.ts` (refactored to use Doc<"inventoryItems"> instead of duplicate interface)
- `convex/marketplaces/brickowl/storeClient.ts` (added rate limiting, resiliency, logging, rollback support)
