...
// { success: boolean, bricklinkInventoryId?: number, error?: {...} }
```
...
function handleSyncResult(result, provider, changeId) {
  ...
  return {
    syncStatus: "synced",
    marketplaceId: result.bricklinkInventoryId || result.brickowlLotId,
  };
}
```
...
- Sync queue updates with marketplace IDs:
  - Updates inventoryItems table with `bricklinkInventoryId` or `brickowlLotId` on successful sync
...
- Simplified `convex/inventory/sync.ts` to trust marketplace client responses:
  ...
  - Sync functions now extract marketplace IDs from provider-specific fields (`bricklinkInventoryId` vs `brickowlLotId`)
